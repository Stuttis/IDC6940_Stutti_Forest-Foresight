...
The dependent variable, *TotalPrice*, was modeled using the randomForest() function in R. The model was trained to predict Sales prices on orders based on the following independent variables (predictors): *OPCO, Product, Substrate, Requested Delivery, Quantity Ordered, Unit Price and Class*.
  ...
  **•	trainData:** The model was trained on trainData (80% data split) that contains the taregt variable and all predictors.
...
## Methods - Mathematical Concepts {.smaller}
...
where𝑦^𝑂𝑂B,𝑖is the average of the predictions for the𝑖-th data point across all trees where that point was not included in the bootstrap sample.
...
**•	Model 1 vs Model 2**
    ...
    - Considering the computational cost of training more trees:
        ...
        - Choose *Model 2* if accuracy is critical. 
        
        - Choose *Model 1* if speed or resource efficiency is more important.
