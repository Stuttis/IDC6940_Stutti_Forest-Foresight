...
### Data Descriptions - Summary {.smaller .scrollable}
```{r}
library(tidyverse)
library(knitr)
...
# str(SaleData)
```
### Data Descriptions - Glimpse {.smaller .scrollable}
```{r}
library(dplyr)
glimpse(SaleData1)
```
### Data Descriptions - Describe {.smaller .scrollable}
```{r}
...
describe(SaleData1)
```
### Data Descriptions - Snapshot of the data {.smaller .scrollable}
```{r}
kable(head(SaleData1))
