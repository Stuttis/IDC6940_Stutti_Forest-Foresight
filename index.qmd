...
```
Just like in the first use case, let's investigate the feature importance details for this model. OPCO at 40.1% has the highest percent increase in mean squared error (%IncMSE) suggesting the model relies heavily on OPCO for predictions and indicating strong influence on `qtyOrdered`. Month at 29.03% and Product at 27.02% also indicate high importance on model's performance. Looking at the Increase in Node Purity, Product has the highest IncNodePurity (100,229,054) suggesting it is most significant for reducing variability in `qtyOrdered`. `OPCO` (71,703,082) and `UnitPrice` (58,979,972) follow right behind. 
...
With the K-fold Cross-Validation, RMSE actually increased. Not the direction we want to go. We could certainly continue tuning our Random Forest model. Let's instead test the XGBoost (Extreme Gradient Boosting) algorithm below using the *xgboost* package in RStudio.
...
The XGBoost significantly helped lower our RMSE. Consistent with what was found in one of the references reviewed, XGBoost algorithm outperforms the traditional modeling approaches with regard to prediction accuracy. [@jain2015sales]  
We will perform one last tuning of our model and use a cumulative sum of qtyOrdered (grouped by OPCO and Product) within the XGBoost model.
...
For reference and easy comparison, **Table 5** below shows the RMSE from all the models run for Use Case 2.
